...
from .account_device import protect_transport_logs
from .const import (CONF_ADDITIONAL_OPERATION_MODES, CONF_CAPABILITY_OVERRIDES,
                    ...
                    CONF_ENTRY_KIND, CONF_KEY, CONF_MAX_CONNECTION_LIFETIME,
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    """Setup Midea Connect device from a config entry."""

    # Ensure the global data dict exists
    hass.data.setdefault(DOMAIN, {})

    # Setup accounts are storage only. AC startup and polling never log in.
    ...
    if device_type == DeviceType.AIR_CONDITIONER and token and key:
        device = PushAirConditioner(ip=host, port=port, device_id=int(id))
